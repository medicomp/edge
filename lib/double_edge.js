...
process.env['EDGE_NATIVE'] = process.env['EDGE_NATIVE'] ||
    (process.env['EDGE_USE_CORECLR'] ? path.join(process.env['EDGE_NATIVE_LIBRARIES_PATH'], 'edge_coreclr.node') : path.join(__dirname, (process.arch === 'x64' ? 'x64' : 'x86'), 'edge_nativeclr.node'));

var edge = require('./edge.js');

var initialize = edge.func({
    assemblyFile: process.env['EDGE_USE_CORECLR'] ? 'EdgeJs' : path.join(__dirname, '..', 'EdgeJs.dll'),
